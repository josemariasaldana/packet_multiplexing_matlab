...
if tipo_trafico > 900
    ...
    % tráfico video TS
    if floor (tipo_trafico/100) == 6
         nombre_carpeta = 'video_TS';
         nombre_archivos_deseado=strcat('.\',nombre_carpeta,'\video_TS_','time_size_user');

        
    else %%%%%% tráfico p2p video
...
for i=2:length(paquetes(:,1))
    ...
    while (paquetes(j,6) <= paquetes(i,1)) && (j < i)
        if politica_buffer ~= 4 %si la política no es time-limited
        ...
        %la política es 4 time-limited. Todos los paquetes se aceptan en principio
        else 
            %si el tiempo que ha pasado desde que llegó hasta que le toca empezar a enviarse es mayor que el tiempo máximo
    ...
    switch( politica_buffer )
        ...
        case 3 %politica_buffer ==3. El paquete entra según un límite en número de paquetes, y no por tamaño
        ...
        case 4 %politica_buffer==4 política "time-limited". El paquete se acepta siempre
...
% si quiero guardar en un fichero de texto los resultados
% tiempo_llegada  tamaño_IP  ident_flujo   aceptado_o_rechazado (1 o 0)
% columna 1       columna 2  columna 3     columna 7
if escribir_fichero_deseado == 1
    %cojo las columnas que me interesan
    paquetes_con_aceptacion = [paquetes(:,1:3) paquetes(:,7)];

    %sólo quiero los paquetes del trafico deseado, así que quito todos los que
    %tengan 1xx en la tercera columna
    paquetes_con_aceptacion = sortrows(paquetes_con_aceptacion, [3 1]);
    s=1;
    while (paquetes_con_aceptacion(s,3) <= 199 )&&(s < size(paquetes_con_aceptacion,1))
       s=s+1; 
    end    
    paquetes_con_aceptacion = paquetes_con_aceptacion(s:size(paquetes_con_aceptacion,1),:);

    % escribo la matriz en un fichero
    dlmwrite(strcat('.\',nombre_carpeta,'\paquetes_con_aceptacion_',datestr(hora_inicio, 'yyyy-mm-dd_HH.MM'),'.txt'), paquetes_con_aceptacion, 'delimiter','\t', 'newline','pc', 'precision',  '%6.0f')
    clear paquetes_con_aceptacion
end
...
if numero_flujos_deseado == 2
    paquetes_resultado_deseado = sortrows (paquetes_resultado_deseado, 3);
    %el primer flujo será el X01: Deseado pequeño
    i=1;
    while paquetes_resultado_deseado (i+1,3) == paquetes_resultado_deseado(i,3)
        i=i+1;
    end
    %ha acabado un flujo
    paquetes_resultado_deseado_1 = paquetes_resultado_deseado(1:i,:);

    %el segundo flujo será el X02: Deseado grande
    paquetes_resultado_deseado_2 = paquetes_resultado_deseado (i+1:size(paquetes_resultado_deseado,1),:);  
end
...
%prob loss
prob_loss_deseado = 1 - mean(paquetes_resultado_deseado(:,7));
if numero_flujos_deseado == 2
    prob_loss_deseado_1 = 1 - mean(paquetes_resultado_deseado_1(:,7));
    prob_loss_deseado_2 = 1 - mean(paquetes_resultado_deseado_2(:,7));
else
    prob_loss_deseado_1 = prob_loss_deseado;
    prob_loss_deseado_2 = 0;
end
