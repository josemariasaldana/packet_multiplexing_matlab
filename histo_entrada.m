...
%ordeno el fichero de entrada según la tercera columna (usuario)
entrada = sortrows (entrada,3);

%creo otra matriz fila, para almacenar los identificadores de cada usuario
usuarios=zeros(1);

%recorro la matriz buscando nuevos usuarios. Añado una columna por cada
%usuario
...
%creo una matriz tiempos_separados. En cada columna pondré los tiempos
%entre paquetes de un usuario
tiempos_separados=zeros(1,length(usuarios));
tamanos_separados=zeros(1,length(usuarios));

%variable para almacenar el número de paquetes de cada usuario
...
for(i=1:length(entrada(:,1))-1)
    if usuarios(u)==entrada(i+1,3) %no cambia el usuario
        ...
        %apunto el tamaño en la columna correspondiente
        tamanos_separados(j,u)=entrada(i,2) + IP_UDP_HEADER;
        %aumento el contador
        j=j+1;
    else %cambia el usuario
        %almaceno el número de paquetes de ese usuario
        num_paquetes(u)=j;
        %cambio de usuario
        u=u+1;
...
%almaceno el número de paquetes del último usuario
num_paquetes(u)=j;

%creo la primera columna con los tiempos del histograma de tiempos
eje_x_tiempo=0:200:max(max(tiempos_separados)); %max calcula una línea, no un valor
...
for(u=1:length(usuarios))
    ...
    %para normalizar se podría hacer
...
%añado una primera fila con los nombres de los usuarios
