...
calcular_jitter_conjunto = 0; %si no hay multiplexion, no hace falta

%si se quiere escribir un fichero con los paquetes de tráfico deseado y si
%se han aceptado o no en el buffer
%no desecha el principio ni el final
% el fichero se llama
% .\_resultados_buffer\paquetes_con_aceptacion_',datestr(hora_inicio, 'yyyy-mm-dd_HH.MM'),'.txt'
escribir_fichero_deseado = 1;
...
%los 900 lo reservo para juegos FPS, que usan UDP
%901 'hlcs_1_dedust';
%902 'hl2cs_dedust';
%903 'halo2';
...
for tipo_trafico=601:601
    
%si quiero separar los resultados del trafico deseado en diferentes flujos,
%poner esta variable a 2. (p.ej trafico grande y pequeño de p2ptv
%si todo el trafico deseado es igual (p.ej. juegos) ponerla a 1
if floor(tipo_trafico/100)==5
    numero_flujos_deseado = 2;
else
    numero_flujos_deseado = 1;
end
...
for num_jugadores=15:5:15 %número de jugadores

%%%%%%%%%%%%%%%%%%%%%%% Parámetros de la multiplexión de juegos %%%%%%%%%%%%%%%%%%%%%
%Si PE o TO o NP o TH valen 0, el tráfico es nativo

%valores_intervalo son los valores de PE o TO que se van a usar en la multiplexión
%valores_intervalo = [0 5 25 50];
%valores_intervalo = [5 15 25];
valores_intervalo = [0];
for w = 1:size(valores_intervalo,2)
    
%Para usar política PERIOD
PE = valores_intervalo(w);
TO = PE;

%Para usar política TIMEOUT
%TO = valores_intervalo(w);
%PE = 100 * TO;
...
for prioridad = 0:0; %no funciona de momento

%política a usar
% 1 "strict": tamaño en bytes fijo estricto. si no cabe entero el paquete, se desecha 
% 2 "one byte": tamaño en bytes fijo. si queda un byte libre en el buffer, el paquete se acepta 
...
for kbits_por_segundo_buffer = 512:1000:512;
bits_por_segundo_buffer = 1000 * kbits_por_segundo_buffer; 

for paq_por_segundo_buffer = 2500000000:1:2500000000; %si es muy grande, no limita
...
numeros_maximo_paq_en_buffer = [0];%descomentar esta linea cuando la política no sea 3
